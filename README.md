Hi, I'm Fred 👋
[📄 View My Resume](./Resume_FirstName_LastName.pdf)
Data-Driven Business Analyst | Quantitative Researcher

I am a professional educator and researcher with a background in Economics and a passion for business analytics. My career is built on the intersection of complex data modeling and clear communication. My goal is to turn raw information into actionable strategy.

🛠️ Technical Toolkit
Data Analysis: STATA/SE, Excel (Advanced Modeling), SQL
Visualization: Power BI, Excel Dashboards
Research & Documentation: LaTeX, Microsoft Word (Technical Writing)
Domain Expertise: Econometrics, Macroeconomic Policy, Sales

🚀 Key Projects
Sales Analysis – An analysis of sales trends, drivers, and profitability across different regions and customer segments using Excel. 
Immigration Enforcement Impacts – An econometric analysis of the 287(g) MOA enforcement on immigrant communities in the USA using STATA/SE.
  
🎓 Education & Upskilling
B.A. in Economics
Certifications: Business Analytics using Excel, Financial & Valuation Modeling, Project Management Tools, Techniques and Agile Frameworks

📫 Let's Connect!
LinkedIn: Profile Link
Location: Chandler, Arizona
Targeting: Business analysis and consulting roles.
